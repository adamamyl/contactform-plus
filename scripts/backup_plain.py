...
# ///
"""
EMF Conduct System plain (unencrypted) database backup script.

Creates a compressed, unencrypted backup of the PostgreSQL database.
Use scripts/backup.py instead if you need age encryption at rest.

Usage:
    python backup_plain.py [--rsync <dest>] [--systemd] [--dry-run]

Output file format: emf_forms-<ISO8601>.dump.zst
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="EMF Conduct plain (unencrypted) database backup",
...
def write_backup(
    ...
    filename = f"emf_forms-{ts}.dump.zst"
...
def generate_systemd_units(dry_run: bool = False) -> None:
    service_content = f"""\
[Unit]
Description=EMF Conduct plain database backup
...
"""
    timer_content = """\
[Unit]
Description=EMF Conduct plain backup timer
...
"""
    service_path = Path("/etc/systemd/system/emf-backup-plain.service")
    timer_path = Path("/etc/systemd/system/emf-backup-plain.timer")

    if dry_run:
        print(f"  [dry-run] Would write {service_path}")
        print(f"  [dry-run] Would write {timer_path}")
        print("  [dry-run] Would run: systemctl enable --now emf-backup-plain.timer")
        return

    service_path.write_text(service_content)
    timer_path.write_text(timer_content)
    _run(["systemctl", "daemon-reload"])
    _run(["systemctl", "enable", "--now", "emf-backup-plain.timer"])
