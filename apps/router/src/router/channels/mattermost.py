...
class MattermostAdapter(ChannelAdapter):
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if resp.status_code == 200:
                    return "mattermost"
                log.warning(
                    "MattermostAdapter.send got %s for case %s", resp.status_code, alert.case_id
                )
                return None
        except Exception:
            log.exception("MattermostAdapter.send failed for case %s", alert.case_id)
            return None
    ...
    async def send_ack_confirmation(self, alert: CaseAlert, message_id: str) -> None:
        ...
        try:
            ...
            log.exception(
                "MattermostAdapter.send_ack_confirmation failed for case %s", alert.case_id
            )
